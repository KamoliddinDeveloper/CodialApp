...
class  MyDbHelper(context: Context): SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION), MyDbInterface {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        val querStudent="create table $STUDENT_TABLE ($STUDENT_ID integer not null primary key autoincrement unique, $STUDENT_NAME text not null, $STUDENT_SURNAME text not null, $STUDENT_PHONE text not null, $STUDENT_DAY text not null, $STUDENT_GROUP_ID integer not null, foreign key($STUDENT_GROUP_ID) references $GROUP_TABLE($GROUP_ID))"
        db?.execSQL(querStudent)
    }
    ...
    override fun addMentor(mentor: Mentor) {
        ...
        contentValues.put(MENTOR_NAME, mentor.surname)
        ...
        if (cursor.moveToFirst()){
            do {
                list.add(Group(
                    id = cursor.getInt(0),
                    name = cursor.getString(1),
                    mentor = getMentorById(2),
                    time = cursor.getString(3),
                    daysOfWeek = cursor.getString(4),
                    course = getCourseById(5),
                    open = false
                )
                )
            }while (cursor.moveToNext())
    ...
    override fun deleteStudent(student: Student) {
        val database = writableDatabase
        database.delete(STUDENT_TABLE, "$STUDENT_ID=?", arrayOf(student.toString()))
        database.close()
    }
    ...
    override fun editGroup(group: Group) {
        ...
        contentValues.put(GROUP_DAYSOFWEEK, group.daysOfWeek)
        contentValues.put(GROUP_COURSE_ID, group.course?.id)
        contentValues.put(GROUP_OPEN, group.id)
    ...
    override fun editStudent(student: Student) {
        ...
        contentValues.put(STUDENT_DAY, student.day)
        contentValues.put(STUDENT_GROUP_ID, student.group?.id)
        database.update(STUDENT_TABLE, contentValues, "$STUDENT_TABLE=?", arrayOf(student.id.toString()))
    }
    ...
    override fun editGroupOpen(group: Group) {
        ...
        contentValues.put(GROUP_COURSE_ID, group.course?.id)
        contentValues.put(GROUP_OPEN, group.open)
        database.update(GROUP_TABLE, contentValues, "$COURSE_ID=?", arrayOf(group.id.toString()))
    }
}